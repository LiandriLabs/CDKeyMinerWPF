...
namespace CDKeyMiner
{
    class Phoenix : IMiner
    {
        ...
        public void Start(Credentials credentials)
        {
            ...
            if (algo == Algo.ETH)
            {
                phoenixProc.StartInfo.Arguments = $"-pool pool.cdkeyminer.com:9000 -wal {credentials.Username} -proto 2 -stales 0 -rate 0 -cdm 0 -gsi 0 -log 1 -logdir Logs";
            }
            else if (algo == Algo.ETC)
            {
                phoenixProc.StartInfo.Arguments = $"-pool pool.cdkeyminer.com:9001 -wal {credentials.Username} -proto 2 -stales 0 -rate 0 -cdm 0 -gsi 0 -log 1 -logdir Logs";
            }
            else
            {
